...
from pr_change_tracker.handlers.github._processors import _pull_request_review
...
class TestPullRequestReviewEvents:
    class TestDismissed:
        def test_dismissed_collab(self, test_logic: PerTestLogic) -> None:
            test_logic.assertFixture("dismissed-collab", _pull_request_review._DismissedEvent())

        def test_dismissed_owner(self, test_logic: PerTestLogic) -> None:
            test_logic.assertFixture("dismissed-owner", _pull_request_review._DismissedEvent())

    class TestSubmitted:
        def test_submitted_approve(self, test_logic: PerTestLogic) -> None:
            test_logic.assertFixture("submitted-approve", _pull_request_review._SubmittedEvent())

        def test_submitted_changes_requested_collab(self, test_logic: PerTestLogic) -> None:
            test_logic.assertFixture(
                "submitted-changes_requested-collab", _pull_request_review._SubmittedEvent()
            )

        def test_submitted_commented_collab(self, test_logic: PerTestLogic) -> None:
            test_logic.assertFixture(
                "submitted-commented-collab", _pull_request_review._SubmittedEvent()
            )

        def test_submitted_commented_owner(self, test_logic: PerTestLogic) -> None:
            test_logic.assertFixture(
                "submitted-commented-owner", _pull_request_review._SubmittedEvent()
            )
